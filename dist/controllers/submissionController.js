...
const formatBackendError = (error) => {
    ...
    return Object.fromEntries(Object.entries(safeError).filter(([, value]) => value !== undefined && value !== null));
};
const shouldShowErrorDetails = () => process.env.SHOW_ERROR_DETAILS === 'true';
const isPlainObject = (value) => {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
};
const extractPatientCode = (record) => {
    const code = record.codePatient;
    return typeof code === 'string' && code.trim() ? code.trim() : null;
};
const extractPatientAgeLabel = (record, patientAge) => {
    const label = record.ageLabel;
    if (typeof label === 'string' && label.trim())
        return label.trim();
    const months = Number(record.ageMonths);
    const unit = record.ageUnit;
    if (unit === 'months' && Number.isFinite(months) && months > 0)
        return `${months} mois`;
    if (record.age80Plus === true || record.age80Plus === 'true')
...
const normalizePayload = (body) => {
    ...
    const selectedPathologies = Array.isArray(body.selectedPathologies)
        ? body.selectedPathologies.filter((item) => typeof item === 'string' && item.trim().length > 0)
        : [];
...
const mapSubmissionForList = (item) => {
...
export const patchSubmissionResponses = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isFinite(id)) {
            return res.status(400).json({ success: false, error: 'Identifiant invalide.' });
...
export const patchSubmissionExamensComplementaires = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isFinite(id)) {
            return res.status(400).json({ success: false, error: 'Identifiant invalide.' });
